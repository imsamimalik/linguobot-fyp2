...
import { Utils, Pose, Hand, Face, Vector, TFace } from "kalidokit";
import { GLTF, GLTFLoader } from "three/examples/jsm/loaders/GLTFLoader.js";

const Model = (props: { getHolisticResults: () => any }) => {
    const clamp = Utils.clamp;
    const lerp = Vector.lerp;

    const gltf = useLoader(GLTFLoader, "./linguobotmv0.vrm", (loader) => {
    ...
    const rigRotation = (
        ...
        const Part = gltf.userData.vrm.humanoid.getRawBoneNode(
    ...
    const rigFace = (riggedFace: TFace) => {
        if (!gltf) {
            return;
        }
    ...
    const animateVRM = (
        ...
        if (pose2DLandmarks && pose3DLandmarks) {
            ...
            // rigRotation("Hips", riggedPose!.Hips.rotation, 1);
            // rigPosition(
            //     "Hips",
            //     {
